...
const useFollowMouseEffect = (
  ...
  useEffect(() => {
    ...
    function moveEffect(e: MouseEvent<Element, MouseEvent>) {
      ...
      const x = Math.round((e.clientX - parentRect.x - (parentRect.width / 2)) / 1.8);
      const y = Math.round((e.clientY - parentRect.y - (parentRect.height / 2)) / 1.8);
    ...
    function leaveEffect() {
      children.forEach((child) => {
        child.current.style.setProperty('transition', 'transform 0.6s cubic-bezier(.50,1.8,.50,.8)');
